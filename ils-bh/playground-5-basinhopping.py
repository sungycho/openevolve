...
def main():
    ...
    print(f"  Phase 2 : top {args.starts} winners x {args.niter} BH steps  "
          f"(stepsize={args.stepsize}, T={args.T})")
    ...
    tasks = [(seed, args.niter, args.stepsize, args.T, x0)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            all_results.append((score, seed, feasible, accepted_steps))
            done = len(all_results)
            tag = "ok" if feasible else "INFEASIBLE"
            best_accepted = max(accepted_steps) if accepted_steps else float("nan")
    ...
    feasible = [(s, seed, steps) for s, seed, ok, steps in all_results if ok]
    ...
    for rank, (score, seed, accepted_steps) in enumerate(feasible, 1):
        delta = score - 2.626891
        sign = "+" if delta >= 0 else ""
        n_accepted = len(accepted_steps)
        best_mid = max(accepted_steps) if accepted_steps else float("nan")
        print(f"  #{rank}  score={score:.6f}  ({sign}{delta:.6f})"
              f"  seed={seed}  accepted={n_accepted}  best_mid={best_mid:.6f}")
